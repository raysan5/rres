...
`rres` has been designed to package game assets (images, fonts, text, audio, models...) data into a simple self-contained comprehensive format, easy to read and write, prepared to load data in a fast and efficient way.
...
rres has been in development since 2014 when I started this project looking for a packaging file-format similar to XNB for raylib. In the last 8 years the project has suffered multiple redesigns and improvements along a learning process about file-formats. In that time I have implemented loaders/writers for +20 different fileformats and also created +12 custom file formats for multiple custom tools.

rres file-format has gone through _at least_ 4 big redesigns:

 - First design of the format was limited to packaging one resource after another, every resource consisted of one info header followed by a fixed set of four possible parameters and the resource data. Along the .rres file, a .h header file was also generated to map the `rresId` with a resource name (usually the original filename of the un-processed data). This model was pretty simple and intuitive but it has some important downsides: not considering complex pieces of data that could require multiple chunks, no way to extract resources to original files (or similar ones).

 - Second design was way more complex and tried to address first design shortcomings. In this design every resource could include multiple chunks of separate data, clearly defined by a set of properties and parameters. Actually, that design was similar to RIFF file-format and how most file types are structure: every resource in the package could be threatened as a separate file on its own. Some additional resource options were but the format become quite complex to understand and manage. Finally the implementation was discarded and a simpler alternative was investigated.

 - Third design was a return to first design simplicity but keeping some of the options for the individual resources. The problem of multiple resources generated from a single input file was solved using a simple offset field in the resource info header that points to next linked resource when required. Resources are loaded as an array of resource chunks. An optional Central Directory resource chunk was added as an option to keep references for the input files. Format was good but it still required implementation and further investigation, it needed to be platform-agnostic and it was still linked to raylib structures and functionality.

 - Fourth design has been done along the implementation, almost all structures and fields have been renamed for consistency and simplicity. A separare library has been created (`rres-raylib.h`) to map the loaded resources data into a custom library/engine data types, any depndency from rres to raylib has been removed, making it a completely engine-agnostic file-format. Several usage examples for raylib has been implemented to illustrate how to connect rres to an external library/example and differents types of resource loading examples have been implemented. `rrespacker` tool has been implemented with a powerful command-line but also a nice gui interface to package rres resources in a very simple way.

It's been an 8 years project on-and-off with many redesigns and revisions but I'm presonally very happy with the result: a packaging file-format at the level of any professional engine packaging format but free and open-source, available to any game developer that wants to use, implement or customize rres packaging.

## rres usefulness

rres data packaging file-format is a great option to package game resources/assets for several reasons:

 - Free and open source: In comparison to other engines using custom private packaging file-formats, rres is free and open-source. Users can implement their own custom packers or use custom versions of the format if desired.

 - Organization: All the game files data could be organized in a single or a few rres files. It keeps the game directory clean and well structured. 

 - Performance: Keeping all required files for a level/map into a single rres file reduces the need of file handles to access multiple files; file handles are an OS resources, and the cost of opening a file is not insignificant and may cause a delay in the game. Data access times inside the rres file is also important and depending how the files are organized inside the rres it could also improve the loading times.

 - Security: Data is packed in a custom rres fileformat, for most users it's very difficult to access that data so the copyrighted materials are protected. rres also supports per-resource data compression and encryption, adding an extra level of security to game assets when required. 

 - Downloading times: On platforms like HTML5 or server data access, downloading the assets packed into a single file improves download times, in comparison to downloading each file individually.
